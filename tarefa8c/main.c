...
float h(float a, float b){
  return (640*a*pow(cos(b),2)-640*a*pow(sin(b),2));
}

float p(float a, float b) {
  return ((-1280)*pow(a,2)*sin(b)*cos(b));
}
...
{	
  ...
  //teste do problema1
  //return sqrt(pow(20*x,2)+pow(20 * y,2)+1);

	return sqrt(pow(h((0.5*(x+1)),(PI*(y+1))),2)+pow(p((0.5*(x+1)),(PI*(y+1))),2)+1);
}
